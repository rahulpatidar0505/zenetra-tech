  ...
  private downloadPdfFromAssets(courseName: string): void {
    ...
    fetch(pdfPath)
      ...
      .then(blob => {
        ...
        // Even if the file is empty, still allow download
        if (blob.size === 0) {
          console.warn('PDF file is empty, but proceeding with download');
        }
      ...
      .catch(error => {
        ...
        // If fetch fails, try direct link approach as fallback
