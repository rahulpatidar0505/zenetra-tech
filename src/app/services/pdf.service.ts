...
export class PdfService {
  ...
  private generateHeader(pdf: jsPDF, courseData: CourseData): number {
    let yPos = this.layout.margin;
    const pageWidth = pdf.internal.pageSize.getWidth();

    // Add header background with gradient effect
    const headerHeight = this.layout.margin * 3;
    ...
    pdf.text(zenetraText, (pageWidth - textWidth) / 2, yPos + 12);

    yPos += this.layout.lineHeight * 4;
    ...
    if (courseData.title) {
      ...
      pdf.roundedRect(this.layout.margin - 10, yPos - 20, pageWidth - (this.layout.margin * 2) + 20, 40, 3, 3, 'F');
      ...
      pdf.text(courseData.title, (pageWidth - titleWidth) / 2, yPos);
    ...
    return yPos + this.layout.lineHeight * 6;
  }
  ...
  private generateCourseContent(pdf: jsPDF, courseData: CourseData, startY: number): number {
    ...
    pdf.setFillColor(this.colors.background.r, this.colors.background.g, this.colors.background.b);
    pdf.roundedRect(
      this.layout.margin - infoBoxMargin, 
      yPos - 10, 
      pageWidth - (this.layout.margin * 2) + (infoBoxMargin * 2), 
      80, 
      5, 
      5, 
      'F'
    ...
    pdf.setFillColor(this.colors.secondary.r, this.colors.secondary.g, this.colors.secondary.b);
    pdf.circle(this.layout.margin, yPos - 3, 3, 'F');
    pdf.setFontSize(this.fonts.subheading.size);
    pdf.setTextColor(this.colors.primary.r, this.colors.primary.g, this.colors.primary.b);
    pdf.setFont('helvetica', this.fonts.subheading.style);
    pdf.text(`Duration: ${courseData.duration}`, this.layout.margin + 10, yPos);
    yPos += this.layout.lineHeight * 2.5;
    ...
    pdf.text("Course Overview", this.layout.margin, yPos);
    yPos += this.layout.lineHeight * 1.5;
    
    // Description content
    pdf.setFont('helvetica', 'normal');
    const descriptionLines = pdf.splitTextToSize(courseData.description, pageWidth - (3 * this.layout.margin));
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.text("Course Modules", this.layout.margin, yPos + 2);
    yPos += this.layout.lineHeight * 3;
    ...
    courseData.modules.forEach((module, index) => {
      ...
      pdf.setFillColor(this.colors.secondary.r, this.colors.secondary.g, this.colors.secondary.b);
      pdf.circle(this.layout.margin + 10, yPos - 4, 8, 'F');
      pdf.setTextColor(this.colors.primary.r, this.colors.primary.g, this.colors.primary.b);
      pdf.setFont('helvetica', 'bold');
      pdf.text(`${index + 1}`, this.layout.margin + 7, yPos - 1);
      pdf.text(module.title, this.layout.margin + 25, yPos);
      yPos += this.layout.lineHeight * 1.5;

      // Module goal with styled box
      pdf.setFillColor(245, 245, 245);
      pdf.setDrawColor(this.colors.secondary.r, this.colors.secondary.g, this.colors.secondary.b);
      pdf.roundedRect(this.layout.margin + 20, yPos - 5, pageWidth - (2.5 * this.layout.margin), 30, 2, 2, 'FD');
      
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(this.colors.text.r, this.colors.text.g, this.colors.text.b);
      pdf.setFontSize(12);
      const goalLines = pdf.splitTextToSize(module.goal, pageWidth - 3 * this.layout.margin);
      pdf.text("Goal:", this.layout.margin + 25, yPos + 5);
      pdf.setFont('helvetica', 'italic');
      pdf.text(goalLines, this.layout.margin + 55, yPos + 5);
      yPos += (this.layout.lineHeight * goalLines.length) + this.layout.lineHeight * 2;
      ...
      module.content.forEach(item => {
        ...
        // Custom bullet point
        pdf.setFillColor(this.colors.primary.r, this.colors.primary.g, this.colors.primary.b);
        pdf.circle(this.layout.margin + 25, yPos - 2, 1.5, 'F');
        
        pdf.setFont('helvetica', 'normal');
        pdf.setTextColor(this.colors.lightText.r, this.colors.lightText.g, this.colors.lightText.b);
        const contentLines = pdf.splitTextToSize(item, pageWidth - 3 * this.layout.margin);
        pdf.text(contentLines, this.layout.margin + 32, yPos);
  ...
  private generateFooter(pdf: jsPDF): void {
    ...
    // Accent line
    pdf.setFillColor(this.colors.secondary.r, this.colors.secondary.g, this.colors.secondary.b);
    pdf.rect(0, pageHeight - this.layout.footerHeight * 2, pageWidth, 1, 'F');
    ...
    // Right side - Contact
    pdf.text("Contact: info@zenetra.com", pageWidth - 80, pageHeight - this.layout.footerHeight);
  ...
  private addPageNumbers(pdf: jsPDF): void {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      pdf.text(pageText, pageWidth - 40, pageHeight - this.layout.footerHeight);
    }
  }

  private checkPageBreak(pdf: jsPDF, currentY: number): number {
    const pageHeight = pdf.internal.pageSize.getHeight();
    if (currentY > pageHeight - this.layout.pageBreakThreshold) {
      pdf.addPage();
      const newY = this.generateHeader(pdf, { title: '', description: '', duration: '', modules: [] });
      this.generateFooter(pdf);
      return newY;
    }
    return currentY;
